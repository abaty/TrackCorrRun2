...
double TrkCorr::getTrkCorr(float pt, float eta, int correction)
{
  ...
  netEff *= eff2[coarseBin][0]->GetBinContent(eff2[coarseBin][0]->GetXaxis()->FindBin(eta),eff2[coarseBin][0]->GetYaxis()->FindBin(pt));
  ...
  netFake *= fake2[coarseBin][0]->GetBinContent(fake2[coarseBin][0]->GetXaxis()->FindBin(eta),fake2[coarseBin][0]->GetYaxis()->FindBin(pt));
...
*/
  ...
  if(1/netEff>1000) std::cout << "problem here!" << netEff <<  " " <<pt << " " << eta <<  " " << coarseBin << std::endl;
