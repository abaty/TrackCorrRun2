...
//    trkCorr->UpdateEventInfo(trkPt,trkEta,trkPhi,nTrk);
    ...
    for(int j = 0; j<nTrk; j++)
    {
      ...
      float fake = trkCorr->getTrkCorr(trkPt[j],trkEta[j],2);
    ...
    for(int j = 0; j<nParticle; j++)
    {
      ...
      if(mtrkPtError[j]/mtrkPt[j]>0.3 ||TMath::Abs(mtrkDz1[j]/mtrkDzError1[j])>3 || TMath::Abs(mtrkDxy1[j]/mtrkDxyError1[j])>3) continue;
      ...
      float eff = trkCorr->getTrkCorr(genPt[j],genEta[j],1);
