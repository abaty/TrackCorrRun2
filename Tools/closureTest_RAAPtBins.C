...
//Setup variables for skim
  ...
  //booking applied corrections
    ...
    TH2D * weightHist = new TH2D("weightHist","weightHist",60,-14,-4,25,0,50);
    TH2D * weightHist2 = new TH2D("weightHist2","weightHist2",60,-14,-4,25,0,50);
...
//  if(nFiles>3) continue;
  ...
  { 
    ...
    {
      //if((trkPt[j]<100 && trkPt[j]>pthat/1.5) || (trkPt[j]>=100 && trkPt[j]>pthat/1.5) || (trkPt[j]>28.8 && trkPt[j]<35.2 && nFiles<3)) continue;
    ...
    {
      //if((genPt[j]<100 && genPt[j]>pthat/1.5) || (genPt[j]>=100 && genPt[j]>pthat/1.5)|| (genPt[j]>28.8 && genPt[j]<35.2 && nFiles<3)) continue;
      ...
      //fill histograms
      //for eff/total
      if(hiBin>=10 && hiBin<20) weightHist2->Fill(TMath::Log10(weight),genPt[j]);       
      ...
      if(hiBin>=10 && hiBin<20) weightHist->Fill(TMath::Log10(weight),genPt[j]);       
  ...
  weightHist->SetDirectory(outF);
  weightHist2->SetDirectory(outF);
  //weightHist->Write();
  outF->Write();
